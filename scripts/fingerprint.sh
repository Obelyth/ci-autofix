...
# Test declarations across the ecosystems these repos actually use.
cases=$(grep -rEIn --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=.ci-autofix \
  "(^|[^a-zA-Z0-9_])(it|test|describe)\s*\(|^\s*def test_|^\s*func Test[A-Z]|^\s*#\[test\]|@Test\b" . 2>/dev/null | wc -l | tr -d ' ')

asserts=$(grep -rEIn --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=.ci-autofix \
  "expect\s*\(|assert[A-Za-z]*\s*\(|\bassert\b|XCTAssert" . 2>/dev/null | wc -l | tr -d ' ')
